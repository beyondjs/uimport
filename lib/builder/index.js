...
module.exports = class {
    ...
    async process() {
        ...
        const {downloads, errors} = await (async () => {
            ...
            console.log('Download the following packages', [...tree.list.keys()]);
            for (const [vname, vpackage] of tree.list) {
                const downloader = new Downloader(vpackage, this.#specs.downloader);
                try {
                    console.log('Downloading', vname);
                    await downloader.process();
                    console.log('Package download completed', vname);
                }
                catch (exc) {
                    console.log(`Error downloading package "${vname}": ${exc.message}`);
