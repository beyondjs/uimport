	...
	async process() {
		try {
			const { errors, warnings, metafile } = await esbuild.build({
				...
				write: false,
				treeShaking: false,
				plugins: [plugin(this.#wrapper, this.#cwd)]
			});
			...
			if (this.#bundle === '@firebase/auth') console.log(2, 'wrapper', this.#wrapper);
			if (this.#bundle === '@firebase/auth') console.log(3, 'cwd', this.#cwd);
