...
module.exports = async function (bundle, paths) {
    ...
    const build = async (def) => {
        ...
        // [...metafile.ims].map(([input, im]) => {
        //     if (!input.includes('/styles/')) return;
        //     console.log(`${input}`.blue);
        //     console.log([...im.consumers].map(consumer => consumer.input));
        // });
    ...
    !errors && await fs.writeFile(paths.cache, code, 'utf8');
