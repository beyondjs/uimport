...
module.exports = async function (bundle, paths) {
    ...
    if (pkg.uimport) {
        const {exports, clientDependencies} = pkg.json;
        ...
        entry = entry.replace('./', '');
        const code = await fs.readFile(p.join(pkg.path, entry), 'utf8');
        const dependencies = clientDependencies ? clientDependencies : [];
