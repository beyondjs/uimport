...
module.exports = async function (bundle, paths) {
    ...
    if (pkg.uimport) {
        //TODO identificar cual es el file a tomar de la carpeta ts de node_modules
        console.log(1, bundle, pkg.name, pkg.uimport, pkg.version, pkg.path)
        const build = await fs.readFile(p.join(pkg.path), 'utf8');
