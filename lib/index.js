 ...
 */
module.exports = async function build(resource, mode, specs) {
	mode = mode ? mode : 'esm';
	specs = specs ? specs : {};
	...
	const { specifier, subpath } = vspecifier;

	// Check if the bundle is already in cache
	const cacheFile = join(specs.cache, specifier, `${vspecifier.version}+${mode}.js`);
	...
	if (await exists(cacheFile)) {
		try {
			...
			return { errors, warnings, pkg: undefined, subpath, code, version: vspecifier.version, dependencies };
		} catch (exc) {
	...
	const { code, version, errors, warnings, dependencies } = await require('./bundle')(pkg, vspecifier, mode, specs);
	...
	if (specs.dependencies) {
		...
		for (const dependency of dependencies) {
			...
			const vspecifier = new VSpecifierParser(dependency.id);
			const { errors } = await build(`${vspecifier.specifier}.js`, mode, { ...specs, cwd });
