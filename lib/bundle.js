 ...
 * @param specs {{cwd: string, cache: string, temp: string, versions:boolean, prePath:boolean}}
 * @return {Promise<{errors: [string]}|any>}
 */
module.exports = async function (pkg, vspecifier, mode, specs) {
	if (!specs || typeof specs.cwd !== 'string' || typeof specs.temp !== 'string' || typeof specs.cache !== 'string') {
	...
	if (pkg.version !== version) {
		const error =
			`The requested version "${version}" differs ` +
	...
	if (pkg.json.uimport && !specs.versions && !specs.prePath) {
	...
	const cacheFile = join(specs.cache, specifier, `${version}+${mode}.js`);
	...
	const { code, errors, warnings, dependencies } = await require('./build')(specifier, version, mode, specs);
